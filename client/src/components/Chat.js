...
function Chat() {
  ...
  const handleChange = (e) => {
    if (e.target.files[0]) {
      if (
        ...
        e.target.files[0].size / 1024 <= 1026
      ) {
  ...
  const lastMessageTimestamp = (messages) => {
    if (messages && messages.length === 0) {
      ...
      return moment(messages[messages.length - 1]?.timestamp).format(
      ...
      <div className="chat_body">
        {messages?.map((message, i) => {
          if (message.messageType === "text") {
            return (
              <p
                key={message + JSON.stringify(Math.random() * 100)}
            ...
            return (
              <div
                key={message.message + JSON.stringify(Math.random() * 100)}
                className={`chat_message ${
                  message.name === user.displayName && "chat_receiver"
                }`}
              >
